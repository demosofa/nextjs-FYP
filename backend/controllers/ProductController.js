...
class ProductController {
	...
	listManagedProduct = async (req, res) => {
		...
		const productCounted =
			await models.Product.countDocuments(filterOptions).lean();
				...
				arrVariant = await Promise.all(
					variants.map(async (item) => {
						...
						const arrOption = await Promise.all(
							options.map(
								async (opt) =>
									(await models.Option.create([{ name: opt }], { session }))[0]
							)
						);
	...
	putImage = async (req, res) => {
		...
		if (newImages.length) {
			...
			const updated = await models.Product.findByIdAndUpdate(req.query.id, {
